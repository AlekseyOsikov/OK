    ...
    var osmeRegions = (function () {
        ...
        function decodeLineBlock (encodedCoordinates) {
            ...
            while (index < byteVectorLength) {
                var position = [clampx(read() * fx + bounds[0][0]), clampy(read() * fy + bounds[0][1])];
        ...
        function getGeometry (regionId, osmeData, options) {
            ...
            for (var objId = 0, objPathLength = paths.length; objId < objPathLength; ++objId) {
                var objPath = paths[objId];
                var objPathCoordinates = [],
                    objWays = [];
                if (objPath.length && typeof objPath[0] == 'number') {
                    objPath = [objPath];
                }
                for (var pathId = 0, pathLength = objPath.length; pathId < pathLength; ++pathId) {
                    var path = objPath[pathId];
                    var pathCoordinates = [],
                        ways = [];
                    ...
                    objPathCoordinates.push(pathCoordinates);
                    objWays.push(ways);
                }
                coordinates.push(objPathCoordinates);
                meta.push(objWays);
            }
